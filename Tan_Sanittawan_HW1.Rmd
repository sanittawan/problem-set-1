...
## still fail with error even with 48 x 4 dim: Error in qr.solve(a) : singular matrix 'a' in solve
```

Unfortunately, I couldn't locate the problem of the model fitting in the multivariate setting. To get around this problem, I demonstrate that using one feature is also possible for clustering. Here, I selected to fit 2 models: one with salary data and the other with total session length data.
