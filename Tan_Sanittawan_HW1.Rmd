...
SLPdf_matrix <- SLPdta_short %>%
                select(5, 6, 7, 8) %>%
                as.matrix()

# check rank
qr(SLPdf_matrix)$rank
s <- solve(t(SLPdf_matrix) %*% SLPdf_matrix)

# fit model
gmm1 <- mvnormalmixEM(SLPdf_matrix,
                    k = 2, epsilon=1e-04)

## fail with error: Error in qr.solve(a) : singular matrix 'a' in solve
```

I couldn't locate the problem of the model fitting in the multivariate setting. To get around this problem, I demonstrate that using one feature is also possible for clustering. Here, I selected to fit 2 models: one with salary data and the other with total session length data.
...
I have trouble fitting the multivariate normal mixture models. No output could be generated here.
